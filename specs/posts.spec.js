...
describe('Posts', () => {
    describe('Create', () => {
        ...
        it('should not add a new post with existing ID', async () => {
            const response = await chakram.post(api.url('posts'), {
                id: 50,
                title: 'title',
                body: 'body',
        ...
        after(async () => {
            if (addedId) {
                await chakram.delete(api.url('posts/' + addedId));
            }
        });
    });
    ...
    describe('Read', () => {
        it('should have posts', async () => {
          ...
          it('should not return post for invalid ID', async () => {
            const response = await chakram.get(api.url('posts/no-id-like-this'));
    ...
    describe('Update', () => {
        it('should update existing post with given data', async () => {
          const updateData = {
            title: 'title',
            body: 'body',
            userId: 111
          ...
          const response = await chakram.put(api.url('posts/50'), updateData);
          expect(response).to.have.status(200);
      
          const post = await chakram.get(api.url('posts/50'));
          expect(post).to.have.json('data', data => {
            expect(data.title).to.equal(updateData.title);
            expect(data.body).to.equal(updateData.body);
            expect(data.userId).to.equal(updateData.userId);
          });
        });
      
        it('should throw error if the post does not exist', async () => {
          const updateData = {
            title: 'title',
            body: 'body',
            userId: 111
          ...
          const response = await chakram.put(api.url('posts/111'), updateData);
          expect(response).to.have.status(404);
        });
      });
    ...
    describe('Delete', () => {
        it('should delete post by ID', async () => {
            const response = await chakram.delete(api.url('posts/50'));
            expect(response).to.have.status(200);
        ...
        it('should throw error if the post does not exist', async () => {
            const response = await chakram.delete(api.url('posts/111'));
            expect(response).to.have.status(404);
        });
    });
});